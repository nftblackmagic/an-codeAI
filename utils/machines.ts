...
export const provisionIPsApp = async (appId: string) => {
  ...
  const provisionSharedIPv4Response = await fetch(
  ...
  const machineResponse = await fetch(
    `https://api.machines.dev/v1/apps/${appId}/machines`,
    {
      ...
      body: JSON.stringify({
        name: machineName,
        config: {
          image: "registry.fly.io/flow-gen-ai:latest",
          env: {
...
export const updateApp = async (appId: string, machineName: string) => {
  const machineResponse = await fetch(
    `https://api.machines.dev/v1/apps/${appId}/machines/${machineName}`,
    {
      method: "PUT",
      headers: {
        Authorization: `Bearer ${flyToken}`,
...
export const sendGraphQLRequest = async (query: string, variables: any) => {
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `GraphQL 请求失败: ${response.status} - ${response.statusText}`
      );
    }
...
export const updateFileList = async (
  ...
  const hasPackageJson = filePath.find((key: any) =>
    key.includes("package.json")
  );
  if (!hasPackageJson && installDependencies) {
    const reinstallResult = await reinstallDependencies(
      appName,
      installDependencies,
      hasPackageJson,
      machine
  ...
  for (const file of files) {
    ...
    if (estimatedSize > MAX_BATCH_SIZE) {
      console.log(
        `Large file ${file.path}: ${(estimatedSize / 1024).toFixed(
          2
        )}KB, adding as separate batch`
      );
      batches.push([file]);
      continue;
    }
  ...
  async function processBatch(batch: Array<{ path: string; content: string }>) {
    try {
      ...
      // 准备批量上传的文件内容
      const batchFileOps = await Promise.all(
        batch.map(async (file) => {
          ...
          const base64CompressedContent =
            arrayBufferToBase64(compressedContent);
      ...
      batchFileOps.forEach((fileOp, index) => {
        ...
        scriptContent += `# Write file ${index + 1}/${batchFileOps.length}: ${
          fileOp.path
        }\n`;
      ...
      if (!executeResponse.ok) {
        ...
        throw new Error(
          `Failed to execute batch script: ${executeResponse.status} - ${errorText}`
        );
      }
      ...
      if (result.exit_code !== 0) {
        console.error(
          `Batch script execution failed with code ${result.exit_code}`
        );
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      try {
        ...
        console.log(
          "batchResults **********",
          result,
          batches[i].map((file: any) => file.path)
        );
  ...
  const totalProcessed = batchResults.reduce(
    (sum, result) => sum + (result.batchSize || 0),
    0
  );

  console.log(
    `Processed ${totalProcessed}/${files.length} files, success: ${allSuccessful}`
  );
...
const ensureMachineReady = async (appName: string) => {
  ...
  while (retryCount < maxRetries) {
    ...
    switch (machine.state) {
      ...
      case "created":
        console.log(
          "Machine is created, waiting for it to start ",
          retryCount * 5000
        );
        ...
        if (retryCount === maxRetries) {
          throw new Error(
            `Machine stuck in created state after maximum retries ${
              retryCount * 4000
            } seconds`
          );
        }
...
export const listMachines: any = async (appId: string) => {
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(
        `Failed to list machines: ${response.status} - ${errorText}`
      );
    }
...
export const startMachine = async (appId: string, machineId: string) => {
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(
        `Failed to start machine: ${response.status} - ${errorText}`
      );
    }
...
export const executeCommand = async (
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(
        `Failed to execute command: ${response.status} - ${errorText}`
      );
    }
...
export const reinstallDependencies = async (
  appId: string,
  installDependencies: string,
  ...
  try {
    ...
    try {
      ...
      const cdResult = await executeCommand(appId, machine.id, [
        "sh",
        "-c",
        "cd /app",
      ]);
      ...
      console.log("installCommand", [
        "sh",
        "-c",
        `cd /app && ${installCommand}`,
      ]);
      // Run npm install in the app directory
      installResult = await executeCommand(appId, machine.id, [
        "sh",
        "-c",
        `cd /app && ${installCommand}`,
      ]);
